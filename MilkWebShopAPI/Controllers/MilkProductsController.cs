...
public class MilkProductsController : ControllerBase
{
	...
	{
		if (_context.MilkProducts == null) return NotFound();
	...
	public async Task<ActionResult<MilkProduct>> GetMilkProduct(Guid id)
	{
		...
		if (milkProduct == null) return NotFound();
	...
	{
		if (id != milkProduct.Id) return BadRequest();
	...
	{
		if (_context.MilkProducts == null) return Problem("Entity set 'AppDbContext.MilkProducts'  is null.");
